import gymnasium as gym
from gymnasium import spaces
import numpy as np
import pandas as pd


class IEEE96OTS(gym.Env):
    """
    DC Optimal Power Flow with Optimal Transmission Switching (OTS)

    Single-step episode (one dispatch + switching decision per episode)

    Network: IEEE RTS-96 (24-bus, 38-line)

    Action space: Box(n_gen + n_line)
        action[0:n_gen]            — generator dispatch in MW (continuous)
        action[n_gen:n_gen+n_line] — line switching z_k in [0, 1],
                                     thresholded at 0.5 to binary
                                     (z_k = 1: line closed, z_k = 0: line open)

    Observation space: Box(1 + n_line + n_gen + n_line)
        [load_scale (1), normalised line flows (n_line),
         normalised generator dispatch (n_gen), line switch state (n_line)]

    Parameters
    ----------
    max_open_lines : int or None
        j in constraint (6). Maximum number of lines allowed to be open
        simultaneously. None means unconstrained (base case).
    """

    def __init__(self, max_open_lines=None):
        super().__init__()
        # GUB parameter j (constraint 6): None = unconstrained
        self.max_open_lines = max_open_lines

        # Generator data
        gen_df = pd.read_excel("RTS96_SystemData_WithWind.xlsx", sheet_name="Generator")
        self.gen_bus = gen_df["Bus Number"].to_numpy() - 1
        self.p_max = gen_df["Capacity (MW)"].to_numpy()
        self.p_min = gen_df["Minimum generation (MW)"].to_numpy()
        self.n_gen = len(self.gen_bus)

        self.seg_1 = gen_df["Seg 1"].to_numpy()
        self.seg_2 = gen_df["Seg 2"].to_numpy()
        self.seg_3 = gen_df["Seg 3"].to_numpy()
        self.seg_4 = gen_df["Seg 4"].to_numpy()

        self.seg_cost_1 = gen_df["Cost 1"].to_numpy()
        self.seg_cost_2 = gen_df["Cost 2"].to_numpy()
        self.seg_cost_3 = gen_df["Cost 3"].to_numpy()
        self.seg_cost_4 = gen_df["Cost 4"].to_numpy()

        self.no_load_cost = gen_df["NoLoadCost"].to_numpy()

        self.max_cost = sum(
            self.no_load_cost[g]
            + self.seg_cost_1[g] * self.seg_1[g]
            + self.seg_cost_2[g] * self.seg_2[g]
            + self.seg_cost_3[g] * self.seg_3[g]
            + self.seg_cost_4[g] * self.seg_4[g]
            for g in range(self.n_gen)
        )

        # Line data
        line_df = pd.read_excel("RTS96_SystemData_WithWind.xlsx", sheet_name="Line")
        self.from_bus = line_df["From Bus"].to_numpy() - 1
        self.to_bus = line_df["To Bus"].to_numpy() - 1
        self.b = line_df["susceptance (pu)"].to_numpy()
        self.f_max = line_df["Capacity (MW)"].to_numpy()
        self.n_line = len(self.from_bus)
        self.b_base = self.b

        # Bus data
        bus_df = pd.read_excel("RTS96_SystemData_WithWind.xlsx", sheet_name="Bus")
        self.peak_demand = bus_df["Load (MW)"].to_numpy()
        self.n_bus = len(self.peak_demand)

        self.demand = self.peak_demand.copy()

        # Load factor
        load_factor_df = pd.read_excel("RTS96_SystemData_WithWind.xlsx", sheet_name="load factor", index_col=0)
        self.load_factor = load_factor_df.loc["Load factor"].to_numpy()

        # Action space: generator dispatch + line switching decisions
        gen_high = np.maximum(self.p_max, self.p_min + 1.0)
        self.action_space = spaces.Box(
            low=np.concatenate([self.p_min,          np.zeros(self.n_line)]).astype(np.float32),
            high=np.concatenate([gen_high,            np.ones(self.n_line)]).astype(np.float32),
            dtype=np.float32
        )

        # Observation space: load_scale + line flows + gen dispatch + switch state
        obs_size = 1 + self.n_line + self.n_gen + self.n_line
        self.observation_space = spaces.Box(
            low=-np.inf, high=np.inf, shape=(obs_size,), dtype=np.float32
        )

        self.current_flows = np.zeros(self.n_line)
        self.current_bus_balance = np.zeros(self.n_bus)
        self.current_dispatch = np.zeros(self.n_gen)
        self.current_switching = np.ones(self.n_line)   # all lines closed initially

        self._needs_reset = True
        self.reset()

    def _build_B(self, z):
        """Build nodal susceptance matrix using only closed lines (z_k = 1)."""
        bz = self.b_base * z                          # zero out open lines
        B = np.zeros((self.n_bus, self.n_bus))
        np.add.at(B, (self.from_bus, self.from_bus),  bz)
        np.add.at(B, (self.to_bus,   self.to_bus),    bz)
        np.add.at(B, (self.from_bus, self.to_bus),   -bz)
        np.add.at(B, (self.to_bus,   self.from_bus), -bz)
        return B

    def compute_line_flows(self, theta, z):
        """Flow on each line; open lines (z_k = 0) carry zero flow (constraint 3/5a/5b)."""
        return self.b_base * (theta[self.from_bus] - theta[self.to_bus]) * z

    def compute_bus_balance(self, P, flows):
        balance = -self.demand.copy()
        np.add.at(balance, self.gen_bus, P)
        np.add.at(balance, self.from_bus, -flows)
        np.add.at(balance, self.to_bus,    flows)
        return balance

    def _piecewise_cost(self, P):
        seg_limits = [self.seg_1, self.seg_2, self.seg_3, self.seg_4]
        seg_costs  = [self.seg_cost_1, self.seg_cost_2, self.seg_cost_3, self.seg_cost_4]

        total = 0.0
        for g in range(self.n_gen):
            cost_g    = self.no_load_cost[g]
            remaining = float(P[g])
            for s_lim, s_cost in zip(seg_limits, seg_costs):
                if remaining <= 0:
                    break
                alloc      = min(remaining, float(s_lim[g]))
                cost_g    += s_cost[g] * alloc
                remaining -= alloc
            total += cost_g
        return total

    def _get_obs(self):
        total_demand = float(np.sum(self.demand))
        peak_total   = float(np.sum(self.peak_demand))
        load_scale   = total_demand / peak_total if peak_total > 0 else 0.0

        flow_ratio = np.zeros(self.n_line)
        for k in range(self.n_line):
            if self.f_max[k] > 0:
                flow_ratio[k] = self.current_flows[k] / self.f_max[k]

        gen_ratio = np.zeros(self.n_gen)
        for g in range(self.n_gen):
            if self.p_max[g] > 0:
                gen_ratio[g] = self.current_dispatch[g] / self.p_max[g]

        obs = np.concatenate([
            [load_scale],
            flow_ratio,
            gen_ratio,
            self.current_switching,   # z_k state appended to observation
        ]).astype(np.float32)
        return np.nan_to_num(obs, nan=0.0, posinf=0.0, neginf=0.0)

    def reset(self, seed=None, options=None):
        super().reset(seed=seed)

        lf_min = float(self.load_factor.min())
        lf_max = float(self.load_factor.max())
        lf = float(self.np_random.uniform(lf_min, lf_max))
        self.demand = self.peak_demand * lf

        self.current_bus_balance = np.zeros(self.n_bus)
        # current_switching is retained from the previous episode so the agent
        # sees the last topology in the next episode's observation,
        # consistent with how current_flows and current_dispatch are handled

        self._needs_reset = False
        return self._get_obs(), {}

    def step(self, action):
        # --- Parse generator dispatch ---
        P = np.array(action[:self.n_gen])
        seg_max = self.seg_1 + self.seg_2 + self.seg_3 + self.seg_4
        P = np.clip(P, self.p_min, np.minimum(self.p_max, seg_max))
        self.current_dispatch = P

        # --- Parse line switching (threshold continuous [0,1] -> binary z_k) ---
        z_raw = np.clip(np.array(action[self.n_gen:self.n_gen + self.n_line]), 0.0, 1.0)
        z = (z_raw >= 0.5).astype(float)

        # Enforce GUB constraint (6): sum(1 - z_k) <= j
        # If too many lines are open, close the ones with the weakest open signal.
        if self.max_open_lines is not None:
            open_indices = np.where(z == 0.0)[0]
            if len(open_indices) > self.max_open_lines:
                # rank open lines by z_raw ascending (most confident opens first)
                ranked = open_indices[np.argsort(z_raw[open_indices])]
                keep_open = ranked[:self.max_open_lines]
                z = np.ones(self.n_line)
                z[keep_open] = 0.0

        self.current_switching = z

        # --- Net injections ---
        injections = -self.demand.copy()
        # for g in range(self.n_gen):
        #     injections[self.gen_bus[g]] += P[g]
        np.add.at(injections, self.gen_bus, P)

        # --- Build B matrix using only closed lines (OTS constraints 5a/5b) ---
        B = self._build_B(z)
        B_reduced = B[1:, 1:]

        # --- DC power flow (bus 0 is slack) ---
        try:
            theta = np.concatenate([[0.0], np.linalg.solve(B_reduced, injections[1:])])
        except np.linalg.LinAlgError:
            self._needs_reset = True
            return self._get_obs(), -20000, True, False, {"error": "singular_B_matrix"}

        # Flows on active lines only (open lines carry zero flow — constraint 3)
        flows = self.compute_line_flows(theta, z)
        self.current_flows = flows

        bus_balance = self.compute_bus_balance(P, flows)
        self.current_bus_balance = bus_balance

        # --- Reward components ---
        # 1. Generation cost, normalised to [0, 1]
        #    Switching is rewarded implicitly: opening lines that relieve
        #    congestion allows cheaper generators to serve load, reducing cost_term.
        generation_cost = self._piecewise_cost(P)
        cost_term = generation_cost / self.max_cost

        # 2. Per-bus power balance penalty
        bus_balance_penalty = np.sum(np.abs(bus_balance)) / float(np.sum(self.peak_demand))

        # 3. Thermal limit penalty (only active lines)
        flow_penalty = sum(
            max(0.0, abs(flows[k]) - self.f_max[k] * z[k]) / self.f_max[k]
            for k in range(self.n_line)
        )

        # No switching penalty: the agent is free to open any line if it lowers cost.
        n_open = int(self.n_line - np.sum(z))

        reward = -(
            1000  * cost_term
            + 15000 * bus_balance_penalty
            + 2000  * flow_penalty
        )

        self._needs_reset = True

        flow_violation_mw = sum(
            max(0.0, abs(flows[k]) - self.f_max[k] * z[k]) for k in range(self.n_line)
        )

        return self._get_obs(), reward, True, False, {
            "bus_balance_mw":    bus_balance.tolist(),
            "flow_violation_mw": float(flow_violation_mw),
            "agent_dispatch":    P.tolist(),
            "lines_open":        int(n_open),
            "switch_state":      z.tolist(),
            "generation_cost":   float(generation_cost),
            "open_line_indices": [int(k) for k in np.where(z == 0.0)[0]],
        }
